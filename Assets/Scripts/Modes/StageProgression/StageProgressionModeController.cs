...
public sealed class StageProgressionModeController : MonoBehaviour
{
    ...
    private void CompleteCurrentStage()
    {
        ...
        _stageRecords.Add(new StageRunRecord(
            _currentStageIndex,
            config.GetStage(_currentStageIndex).stageId,
            config.GetStage(_currentStageIndex).stageName,
            _stageCorrectCount,
            _stageWrongCount,
            stageElapsed,
            true));
    ...
    private void EndRunSuccess()
    {
        ...
        StageDefinition stage = config.GetStage(_currentStageIndex);
        string difficultyName = stage != null ? stage.stageName : string.Empty;
        WasteSessionSummary summary = _analytics.BuildSummary(
            ...
            mistakeSummaryText: stats.BuildSummaryText(_allStagesCleared, difficultyName),
            clearedStageCount: _allStagesCleared ? 1 : 0,
    ...
    private void EndRunFailed()
    {
        ...
        WasteSessionSummary summary = _analytics.BuildSummary(
            ...
            mostMistakenItemName: stats.MostMistakenItemName,
            mostMistakenItemCount: stats.MostMistakenItemCount,
            mistakeSummaryText: stats.BuildSummaryText(false, difficultyName),
    ...
    private Action ResolveRestartAction()
    {
        ...
        return WasteGameBootstrap.Instance != null
            ? WasteGameBootstrap.Instance.ReturnToStartMenu
            : null;
    }
    ...
    private float SumConfiguredStageTime()
    {
        float total = 0f;
    ...
    {
        StageDefinition stage = config.GetStage(_currentStageIndex);
        string stageLabel = stage != null ? stage.stageName : "标准闯关";
...
public sealed class StageProgressionSessionStats
{
    ...
    public string BuildSummaryText(bool allCleared, string difficultyName)
    {
        ...
        {
            return "「" + label + "」未达成目标。最容易混淆的是 " + MostMistakenItemName + "，建议重点复习后再挑战。";
        }
